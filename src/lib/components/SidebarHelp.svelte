<script lang="ts">
  ...
  const helpSections = $derived([
    ...
    // ── 2: SOUND ──
    {
      ...
      body: L === 'ja'
        ? '最大16トラック (+で追加、REMOVEで削除)\nドックの DRUM/SYNTH/SMPL から楽器変更\nM ミュート、S ソロ (複数可)\nInsert FX — 2スロット直列チェーン (verb/delay/glitch/dist)'
        : 'Up to 16 tracks (+ to add, REMOVE to delete)\nDock tabs: DRUM/SYNTH/SMPL\nM mute, S solo (additive)\nInsert FX — 2-slot serial chain (verb/delay/glitch/dist)',
    },
    {
      category: 2,
    ...
    },
    {
      ...
      docsUrl: '/docs/scene/nodes/',
      body: L === 'ja'
        ? 'ノードグラフで曲構成。エッジで再生順を定義\nルートから開始、分岐ランダム、末端で停止\nノード: Pattern / Generator / Modifier / Sweep\nGenerator: Turing Machine / Quantizer / Tonnetz\nModifier (Transpose/Tempo/Repeat/FX): バブルメニューから追加'
        : 'Node graph for arrangement. Edges define playback order\nStarts at root, random at forks, stops at terminals\nNodes: Pattern / Generator / Modifier / Sweep\nGenerators: Turing Machine / Quantizer / Tonnetz\nModifiers (Transpose/Tempo/Repeat/FX): add from bubble menu',
    },
    // ── 5: PERFORMANCE ──
    {
      category: 5,
