---
title: Function Node Modifiers
description: Modify playback per-node with transpose, tempo, repeat, and FX.
sidebar:
  order: 2
---
...
**Function nodes** are modifier nodes in the scene graph that change how a pattern plays without modifying the pattern itself. They attach as **satellites** to pattern nodes — click a pattern node while in placement mode, and the function node icon appears above it.

Add function nodes from the **tool palette** (circular buttons at the top of the scene) and click a pattern node to attach. Each function type has a distinctive icon: ♪↕ (transpose), ⟳ (repeat), metronome (tempo), ✦ (FX).
...
Function nodes use a **satellite model** instead of manual edge wiring:

- **Attach**: Select a function type from the tool palette, then click a pattern node
- **Detach**: Drag the satellite icon away from its parent pattern
- **Reattach**: Drop a detached function node onto a different pattern
...
Select a function node in the scene view to edit its parameters in the **Dock panel**. Parameters are also accessible from the **SCENE tab** in the Dock when viewing the parent pattern's step sequencer.

## Related

- [Scene Nodes](/docs/scene/nodes/) — node types and edges
- [Playback](/docs/scene/playback/) — how function nodes interact with the engine
