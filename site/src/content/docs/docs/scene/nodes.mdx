...
Creates musical material algorithmically in real-time. Add from the **tool palette** (circular buttons with colored accent rings) at the top of the scene, or **double-click** (long-press on mobile) empty space to open the bubble menu. Connecting a generative node to a pattern **auto-generates** results immediately.
...
- [Function Node Modifiers](/docs/scene/decorators/) — modify node playback
