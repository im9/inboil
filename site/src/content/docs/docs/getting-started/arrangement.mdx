...
Now place your colors on the canvas. In the app you **drag a pattern cell from the Matrix** and **drop it into the Scene** — a new node appears where you release. Then connect nodes by dragging from one node's edge handle to another.

Below, three patterns are already placed and wired into a loop: Verse → Chorus → Break → back to Verse. Try dragging the nodes around, or drag a pattern from the Matrix on the left into the scene:
...
- [Decorators](/docs/scene/decorators/) — transpose, tempo changes, repeats, and automation
