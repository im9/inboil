...
export function initTutorialPatterns() {
  ...
  const defs: [FDef, string, number][] = [[LOFI, 'Verse', 0], [LFB, 'Chorus', 1], [BREAK, 'Break', 2]]
  for (let p = 0; p < 3; p++) {
...
export function initTutorialScene() {
  ...
  song.scene.nodes = [
    { id: 'n1', type: 'pattern', patternId: song.patterns[0]?.id ?? 'pat_00', x: 0.4, y: 0.4, root: true,
      decorators: [{ type: 'repeat', params: { count: 2 } }, { type: 'fx', params: { verb: true, glitch: true } }] },
    { id: 'n2', type: 'pattern', patternId: song.patterns[1]?.id ?? 'pat_01', x: 0.6, y: 0.4,
      decorators: [{ type: 'repeat', params: { count: 2 } }, { type: 'fx', params: { verb: true, glitch: true } }] },
    { id: 'n3', type: 'pattern', patternId: song.patterns[2]?.id ?? 'pat_02', x: 0.5, y: 0.6,
      decorators: [{ type: 'fx', params: { verb: true, glitch: true } }] },
  ]
  song.scene.edges = [
    { id: 'e1', from: 'n1', to: 'n2', order: 0 },
    { id: 'e2', from: 'n2', to: 'n3', order: 0 },
    { id: 'e3', from: 'n3', to: 'n1', order: 0 },
  ]
  song.scene.labels = []
}
